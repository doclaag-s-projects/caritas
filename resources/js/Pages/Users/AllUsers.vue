...
const openUserModal = ( user = null ) => {
  ...
  if ( user ) {
    ...
    // basicPermissions = [
    //     {
    //         id: user.permissions[0].estado === 1 ? true : false,
    //         name: user.permissions[0].crear
    //     },
    //     {
    //         id: user.permissions[0].estado === 1 ? true : false,
    //         name: user.permissions[1].editar
    //     },
    //     {
    //         id: user.permissions[0].estado === 1 ? true : false,
    //         name: user.permissions[2].eliminar
    //     }
    // ]
    ...
    basicPermissions = [
      {
        id: user.permissions[ 0 ].estado,
        name: user.permissions[ 0 ].crear
      },
      {
        id: user.permissions[ 1 ].estado,
        name: user.permissions[ 1 ].editar
      },
      {
        id: user.permissions[ 2 ].estado,
        name: user.permissions[ 2 ].eliminar
      }
    ];
  } else {
...
<template>
                ...
                <tr v-for="      user in paginatedUsers      " :key=" user.id " class="hover:bg-gray-50">
                  ...
                  <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <div class="flex justify-end space-x-2">
                      <button @click="handleUpdate( user )" class="text-blue-600 hover:text-blue-900">
                        <img src="/img/edit.svg" alt="Delete" class="w-6 h-6">
                      </button>
                      <DeleteModal :id=" user.id " :name=" user.name " :option=" 'user' " @delete=" handleDelete " />
                    </div>
                  </td>
                </tr>
              </tbody>
  ...
  <div class="mt-6">
    ...
    <div class="space-y-4">
      <div v-for="permission in basicPermissions" :key="permission.id" class="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
        <span class="text-sm font-medium text-gray-700">{{ permission.name }}</span>
        <label class="relative inline-flex items-center cursor-pointer">
          <!-- Checkbox con estado dinámico basado en userForm.permissions -->
          <input
            type="checkbox"
          ...
          <!-- Div del switch con clases condicionales para movimiento y color -->
          <div :class="`w-11 h-6 rounded-full peer peer-focus:ring-4 peer-focus:ring-blue-300 after:content-[''] after:absolute after:top-0.5 after:left-[2px] after:bg-white after:border after:rounded-full after:h-5 after:w-5 after:transition-all ${
